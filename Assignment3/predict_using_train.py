...
def score(threshold, train_loader, device, model, labels, alpha, gamma):
    threshold = float(threshold)
    predictions_done = make_predictions(threshold, train_loader, device, model)
    prediction_set = np.zeros(100)
    accuracy_for_class = np.zeros(100)
    
    # print(len(predictions))
    for i in range(len(predictions_done)):
        if predictions_done[i] != -1:
            prediction_set[predictions_done[i]] += 1
            if predictions_done[i] == labels[i]:
                accuracy_for_class[predictions_done[i]] += 1
            
    for i in range(100):
        # print(accuracy_for_class[i])
        if prediction_set[i] > 0:
            accuracy_for_class[i] = accuracy_for_class[i]/prediction_set[i]
    ...
    print(threshold, high_acc_count, low_acc_count)
...
def main():
    ...
    submission.to_csv('submission_using_train.csv', index=False)
